...
@router.get("/teacher/overview", response_model=Dict[str, Any], tags=["Teacher Overview"])
async def get_teacher_overview(
    subject: Optional[str] = Query(None, description="Filter by subject"),
    current_user: Dict[str, Any] = Depends(get_current_user)
) -> Dict[str, Any]:
    """
    Get a comprehensive overview of all assessments and configs for the current teacher.
    
    This endpoint provides a single response containing:
    - All assessment configurations created by the teacher
    - All assessments generated by the teacher
    - Summary statistics
    ...
    Returns:
        ...
        - configs: List of AssessmentConfig objects
        - assessments: List of Assessment objects
        - summary: Statistics about configs and assessments
    ...
    try:
        # Get both configs and assessments concurrently
        configs = await assessment_service.get_teacher_assessment_configs(
            teacher_uid=teacher_uid,
            subject_filter=subject
        )
        ...
        # Count configs by subject
        for config in configs:
            subject_name = config.subject
            if subject_name not in subjects_summary:
                subjects_summary[subject_name] = {"configs": 0, "assessments": 0}
            subjects_summary[subject_name]["configs"] += 1
        ...
        for assessment in assessments:
            ...
            if subject_name not in subjects_summary:
                subjects_summary[subject_name] = {"configs": 0, "assessments": 0}
            subjects_summary[subject_name]["assessments"] += 1
        
        # Recent activity (last 5 items)
        recent_configs = sorted(configs, key=lambda x: x.created_at, reverse=True)[:5]
        recent_assessments = sorted(assessments, key=lambda x: x.created_at, reverse=True)[:5]
        ...
        response = {
            ...
            "summary": {
                ...
                "subjects_breakdown": subjects_summary,
        ...
        logger.info(
            f"Retrieved teacher overview for {teacher_uid}: "
            f"{total_configs} configs, {total_assessments} assessments"
        )
        
        return response
        
    except Exception as e:
        logger.error(f"Failed to retrieve teacher overview: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to retrieve teacher overview: {str(e)}"
        )
